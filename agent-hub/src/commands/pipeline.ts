...
import { getAgent, getAgentList, resolveAgentBinary, AgentSpec } from '../agents/registry.js';
...
async function executeAgentPrompt(agent: AgentSpec, prompt: string, timeoutMs: number = 180000): Promise<string> {
...
export async function runPipeline(options: PipelineOptions = {}): Promise<void> {
  ...
  console.log(
    boxen(
      `${chalk.bold.hex('#10B981')('🚀 Multi-Agent Workflow Pipeline 🚀')}\n\n` +
  ...
  console.log(
    boxen(
      `${chalk.bold.hex(architect.color)(`Stage 1: ${architect.name} (Architect)`)}\n\n${architectOutput.trim()}`,
      { padding: 1, margin: { top: 1, bottom: 0 }, borderColor: architect.color as any, borderStyle: 'single' }
    )
  );

  console.log(
    boxen(
      `${chalk.bold.hex(developer.color)(`Stage 2: ${developer.name} (Implementer)`)}\n\n${developerOutput.trim()}`,
      { padding: 1, margin: { top: 1, bottom: 0 }, borderColor: developer.color as any, borderStyle: 'single' }
    )
  );

  console.log(
    boxen(
      `${chalk.bold.hex(reviewer.color)(`Stage 3: ${reviewer.name} (Reviewer)`)}\n\n${reviewerOutput.trim()}`,
      { padding: 1, margin: { top: 1, bottom: 1 }, borderColor: reviewer.color as any, borderStyle: 'single' }
    )
  );
