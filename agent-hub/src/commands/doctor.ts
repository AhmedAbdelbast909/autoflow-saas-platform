...
export async function runDoctor(): Promise<void> {
  ...
  console.log(
    boxen(
      `${chalk.bold.hex('#38BDF8')('⚡ Agent Hub Diagnostics & Health Doctor ⚡')}\n` +
  ...
  for (const r of results) {
    ...
    console.log(
      `${chalk.bold(r.name.padEnd(20))} [${statusIcon}]  ` +
  ...
  if (allGood) {
    console.log(
      boxen(chalk.green.bold('🎉 All 6 AI Coding CLIs are ready and operational!'), {
        padding: 1,
        borderColor: 'green',
        borderStyle: 'single'
      })
    ...
    console.log(
      boxen(chalk.yellow.bold('⚠️ Some agents were not detected. Run their installers to restore them.'), {
        padding: 1,
        borderColor: 'yellow',
        borderStyle: 'single'
      })
