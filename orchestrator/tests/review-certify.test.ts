...
describe("certification policy", () => {
  ...
  it("blocks on P0", () => {
    const d = certificationDecision(DEFAULT_CONFIG,
      [{ id: "X", severity: "P0", problem: "p", required_fix: "f" }], passGates, "FAIL");
    assert.equal(d.certifiable, false);
  });
  it("blocks on P1", () => {
    const d = certificationDecision(DEFAULT_CONFIG,
      [{ id: "X", severity: "P1", problem: "p", required_fix: "f" }], passGates, "FAIL");
    assert.equal(d.certifiable, false);
  });
  it("blocks on test failure", () => {
    const gates = passGates.map((g) => g.name === "tests" ? { ...g, status: "FAIL" as const, exitCode: 1 } : g);
  ...
  it("blocks on typecheck/lint/build failure", () => {
    for (const name of ["typecheck", "lint", "build"]) {
      const gates = passGates.map((g) => g.name === name ? { ...g, status: "FAIL" as const, exitCode: 1 } : g);
      const d = certificationDecision(DEFAULT_CONFIG, [], gates, "PASS");
      assert.equal(d.certifiable, false, name);
    }
  });
  it("SKIPPED required gate blocks certification", () => {
    const gates = passGates.map((g) => g.name === "tests" ? { ...g, status: "SKIPPED" as const, exitCode: null } : g);
    const d = certificationDecision(DEFAULT_CONFIG, [], gates, "PASS");
    assert.equal(d.certifiable, false);
  });
});
