...
export interface OrchestratorConfig {
  ...
  coding: { models: ModelEntry[] };
  review: ReviewerConfig & { provider: string; model: string };
...
export interface RunStateDoc {
  ...
  findings: ReviewFinding[];
  tests: GateResult[];
  history: Array<{ state: RunState; at: string; note: string }>;
  modelsUsed: string[];
  fallbacks: Array<{ from: string; to: string; reason: string; at: string }>;
  gitHead: string | null;
...
export interface OrchestratorCapabilities {
  ...
  reviewer: { executableFound: boolean; executable: string };
}
