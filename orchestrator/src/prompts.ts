...
export type PromptName = "planner" | "implementer" | "reviewer" | "fixer" | "certifier";

const BUILT_IN: Record<PromptName, string> = {
...
export async function loadPrompt(repoRoot: string, name: PromptName): Promise<string> {
  const candidates = [
  ...
  for (const p of candidates) {
    if (existsSync(p)) {
      try { return await readFile(p, "utf8"); } catch { /* fall through */ }
    }
  }
