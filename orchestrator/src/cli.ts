...
import type { CodingAgent, ModelEntry } from "./types.js";
...
async function getConfig(configFlag?: string) {
  const { config, path } = await loadConfig(configFlag, repoRoot);
  return { config: applyEnvOverrides(config), path };
}

async function buildRouterAndReviewer(configPath?: string, runIdForLogs = "doctor") {
...
async function cmdDoctor(configFlag?: string): Promise<number> {
...
async function cmdModels(configFlag?: string): Promise<number> {
...
async function cmdRuns(): Promise<number> {
  ...
  if (ids.length === 0) { console.log("(no runs)"); return 0; }
  for (const id of ids) {
...
async function cmdStatus(runId?: string): Promise<number> {
  const ids = listRunIds(repoRoot);
  const id = runId ?? ids[0];
  if (!id) { console.log("(no runs)"); return 1; }
  const p = runPaths(repoRoot, id);
  const st = await readState(p);
  if (!st) { console.log(`no state for run ${id}`); return 1; }
  ...
  console.log(renderStatus(st, router.snapshot(), {
    ...
    lastAction: String((last?.["type"] as string) ?? st.state),
  }));
  return 0;
}

async function cmdStart(taskFile: string, configFlag?: string, runId?: string): Promise<number> {
  const absTask = resolve(repoRoot, taskFile);
  if (!existsSync(absTask)) { console.error(`task file not found: ${absTask}`); return 1; }
  const { config, path: cfgPath } = await getConfig(configFlag);
  const errors = validateConfig(config);
  if (errors.length > 0) { console.error("invalid config: " + errors.join("; ")); return 1; }
  const id = runId ?? newRunId("run");
  const paths = runPaths(repoRoot, id);
  await ensureRunDirs(paths);
  const caps = { openCode: await detectOpenCode(), reviewer: { executableFound: await reviewerExecutableExists(config.review.executable), executable: config.review.executable } };
  if (!caps.openCode.installed) { console.error("opencode executable not found on PATH"); return 1; }
  console.log(`config: ${cfgPath ?? "(defaults)"}  run: ${id}`);
  const router = new ModelRouter(config.coding.models, { cooldownMs: config.orchestrator.modelCooldownMs });
  const codingAgentFactory = (model: ModelEntry): CodingAgent =>
    new OpenCodeAdapter({ timeoutMs: config.orchestrator.openCodeTimeoutMs, logDir: paths.opencodeDir, repoRoot, caps: caps.openCode });
...
async function cmdResume(runId: string, configFlag?: string): Promise<number> {
  const paths = runPaths(repoRoot, runId);
  const st = await readState(paths);
  if (!st) { console.error(`no state for run ${runId}`); return 1; }
  if (st.finalized) { console.log(`run ${runId} already finalized (${st.state})`); return 0; }
  return cmdStart(st.taskFile, configFlag, runId);
}

async function cmdStop(runId: string): Promise<number> {
  const paths = runPaths(repoRoot, runId);
  const st = await readState(paths);
  if (!st) { console.error(`no state for run ${runId}`); return 1; }
...
async function main(): Promise<number> {
  ...
  if (!cmd || has("--help") || has("-h")) { console.log(usage()); return 0; }
  if (cmd === "init") { await ensureDefaultConfig(); return 0; }
  ...
  if (cmd === "stop") {
    ...
    if (!id) { console.error("stop requires --run-id"); return 1; }
    return cmdStop(id);
  }
  if (cmd === "resume") {
    const id = arg("--run-id") ?? arg("--id");
    if (!id) { console.error("resume requires --run-id"); return 1; }
    return cmdResume(id, arg("--config"));
  }
  if (cmd === "start") {
    const task = arg("--task");
    if (!task) { console.error("start requires --task <file>"); return 1; }
