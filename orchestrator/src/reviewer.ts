...
export class DeepSeekHarnessReviewer implements Reviewer {
  ...
  async review(input: ReviewInput): Promise<ReviewResult> {
    ...
    if (this.opts.model && !args.some((a) => a.includes(this.opts.model as string))) {
  ...
  private spawnReviewer(args: string[], prompt: string): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      try {
        (child.stdin as unknown as { write(s: string): void; end(): void }).write(prompt);
...
export async function reviewerExecutableExists(executable: string): Promise<boolean> {
