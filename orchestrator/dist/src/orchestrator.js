...
import { emitEvent, ensureRunDirs, readState, writeState } from "./state-store.js";
import { redactSecrets } from "./secrets.js";
function parseTaskFile(text, taskFile) {
    ...
    if (taskFile.endsWith(".json")) {
        try {
            ...
            const acceptance = Array.isArray(obj["acceptanceCriteria"]) ? obj["acceptanceCriteria"].map(String) : [];
    ...
    for (const l of lines) {
        if (/acceptance criteria/i.test(l)) {
            inAccept = true;
            continue;
        }
        if (inAccept && /^\s*#{1,3}\s/.test(l))
            break;
        if (inAccept) {
...
function transition(state, next, note) {
    state.state = next;
    state.history.push({ state: next, at: new Date().toISOString(), note });
}
export function certificationDecision(config, findings, gates, reviewerStatus) {
    const blockers = [];
    const p0 = findings.filter((f) => f.severity === "P0").length;
    const p1 = findings.filter((f) => f.severity === "P1").length;
    if (p0 > config.certification.maxP0)
        blockers.push(`P0 findings: ${p0} > allowed ${config.certification.maxP0}`);
    if (p1 > config.certification.maxP1)
        blockers.push(`P1 findings: ${p1} > allowed ${config.certification.maxP1}`);
    if (reviewerStatus !== "PASS")
        blockers.push(`reviewer status is ${reviewerStatus ?? "unknown"} (requires PASS)`);
    const gateByName = new Map(gates.map((g) => [g.name, g]));
    const req = (name, flag) => {
        if (!flag)
            return;
        const g = gateByName.get(name);
        if (!g || g.status === "SKIPPED")
            blockers.push(`${name} gate did not run (SKIPPED) — cannot certify`);
        else if (g.status === "NOT_TESTED")
            blockers.push(`${name} gate NOT_TESTED — cannot certify`);
        else if (g.status !== "PASS")
            blockers.push(`${name} gate status ${g.status} — cannot certify`);
    };
...
async function writeFinalReport(paths, state, status, blockers, extra = {}) {
    const report = {
        status,
        taskId: state.taskId,
        runId: state.runId,
...
function fixerInstructions(findings, requiredActions) {
...
export async function runAutonomousTask(opts) {
    ...
    while (true) {
        ...
        if (state.state === "IMPLEMENT" || state.state === "FIX") {
            ...
            while (attempts < config.orchestrator.maxModelAttempts) {
                ...
                if (!state.modelsUsed.includes(model.model))
                    state.modelsUsed.push(model.model);
                ...
                state.attempt += 1;
                if (codingResult.sessionId)
                    state.sessionId = codingResult.sessionId;
        ...
        if (state.state === "REVIEW") {
            ...
            try {
                ...
                await emitEvent(paths, "REVIEW_ERROR", { error: redactSecrets(String(err.message)).slice(0, 2000) });
                state.consecutiveFailures += 1;
                await writeState(paths, state);
                if (state.consecutiveFailures >= config.orchestrator.maxConsecutiveFailures) {
                    transition(state, "BLOCKED", "reviewer repeatedly unavailable");
                    await writeState(paths, state);
                    const rp = await writeFinalReport(paths, state, "BLOCKED", [`Reviewer unavailable: ${String(err.message).slice(0, 500)}`]);
                    return { finalStatus: "BLOCKED", reportPath: rp, runId: opts.runId };
                }
