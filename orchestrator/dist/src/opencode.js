...
export async function detectOpenCode() {
    const base = { installed: false, version: null, supportsRun: false, supportsModelFlag: false, supportsSessionFlag: false, supportsContinueFlag: false, supportsFormatJson: false };
    const ver = await (async () => {
        const r = await execFileSafe("opencode", ["--version"], { timeout: 15000 });
        if (r.error || (r.code !== 0 && !r.stdout.trim() && !r.stderr.trim()))
            return null;
...
export class OpenCodeAdapter {
    ...
    async run(input) {
        ...
        if (this.opts.caps.supportsFormatJson)
    ...
    spawnOpencode(args, timeoutMs, logFile) {
