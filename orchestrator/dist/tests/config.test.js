    ...
    it("applies ORCH_REVIEW_EXEC/ARGS/TIMEOUT_MS overrides", () => {
        ...
        try {
            ...
            if (prevExec === undefined)
                delete process.env["ORCH_REVIEW_EXEC"];
            else
                process.env["ORCH_REVIEW_EXEC"] = prevExec;
            if (prevArgs === undefined)
                delete process.env["ORCH_REVIEW_ARGS"];
            else
                process.env["ORCH_REVIEW_ARGS"] = prevArgs;
            if (prevTimeout === undefined)
                delete process.env["ORCH_REVIEW_TIMEOUT_MS"];
            else
                process.env["ORCH_REVIEW_TIMEOUT_MS"] = prevTimeout;
        }
    });
    it("loads JSON config with string model list", async () => {
        const dir = mkdtempSync(join(tmpdir(), "cfg-"));
        mkdirSync(join(dir, ".ai", "orchestrator", "config"), { recursive: true });
        writeFileSync(join(dir, ".ai", "orchestrator", "config", "orchestrator.config.json"), JSON.stringify({ coding: { models: ["opencode/a", "opencode/b"] } }));
