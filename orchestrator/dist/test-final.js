...
// Mock reviewer that returns UNAVAILABLE (simulates DeepSeek Harness not installed)
...
async function main() {
    ...
    const workdir = resolve(repoRoot, ".ai", "orchestrator", "test-final");
...
`);
    ...
    const runId = "test-final-" + Date.now();
    ...
    if (state) {
        ...
        console.log(`Reviewer status: UNAVAILABLE (expected)`);
    }
    ...
    if (MockSuccessCodingAgent.calls.length !== 0)
        throw new Error("Resume should not re-run coding agent when already BLOCKED");
    console.log("\n=== ALL TESTS PASSED ===");
    process.exit(0);
}
main().catch((err) => {
